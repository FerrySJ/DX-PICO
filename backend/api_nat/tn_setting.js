...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - TN - Running data setting cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailySettingReport = async () => {
    try {
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                const check = await sequelize.query(
                  `
                    SELECT [process]
                            ...
                            ,[registered_at]
                        FROM [NAT_DX_TO_PICO].[dbo].[TN_SETTING]
                    WHERE machine_name = ?
                  `,
                  {
                    replacements: [mc_no],
                ...
                if (check.length === 0) {
                  ...
                  await sequelize.query(
                    `
                    INSERT INTO [NAT_DX_TO_PICO].[dbo].[TN_SETTING] ([process], [line_name], [machine_name], [machine_order], [shift1_start_time], [count_factor], [target_cycle_time_ms], [registered_at])
                    VALUES (?, ?, ?, ?, ?, ?, ?, GETDATE())
                    `,
                    {
                  ...
                  if (!isSame) {
                    ...
                    await sequelize.query(
                      `
                      DELETE FROM [NAT_DX_TO_PICO].[dbo].[TN_SETTING] WHERE machine_name = ?;
                      `,
                      {
                        replacements: [mc_no],
                      }
        ...
        console.log("NAT - TN - status insert error:" , error);
        return {
            data: error.message,
            success: true,
