...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - AOD - Running data setting cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
    ...
    try {
        let data = await dbNAT.query(`
            WITH [rn] AS (
                ...
                FROM [nat_mc_assy_aod].[dbo].[DATA_PRODUCTION_AOD] a
                LEFT JOIN [nat_mc_assy_aod].[dbo].[DATA_MASTER_AOD] b ON a.mc_no = b.mc_no
            )
            SELECT * FROM [rn]
            where rn = 1
            ORDER BY [mc_no]
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                const check = await sequelize.query(
                  ...
                  SELECT [process]
                      ...
                      ,[registered_at]
                  FROM [NAT_DX_TO_PICO].[dbo].[AOD_SETTING]
                  WHERE machine_name = ?
                  `,
                  {
                    replacements: [mc_no],
                ...
                if (check.length === 0) {
                  ...
                  await sequelize.query(
                    `
                    INSERT INTO [NAT_DX_TO_PICO].[dbo].[AOD_SETTING] ([process], [line_name], [machine_name], [machine_order], [shift1_start_time], [count_factor], [target_cycle_time_ms], [registered_at])
                    VALUES (?, ?, ?, ?, ?, ?, ?, GETDATE())
                    `,
                    {
                  ...
                  if (!isSame) {
                    console.log("NAT - AOD - !isSame", !isSame);
                    
                    // ไม่เหมือนกัน → del แล้ว Insert ใหม่
                    await sequelize.query(
                      `
                      DELETE FROM [NAT_DX_TO_PICO].[dbo].[AOD_SETTING] WHERE machine_name = ?;
                      `,
                      {
                        replacements: [mc_no],
                      }
        ...
        console.log("NAT - AOD - status insert error:" , error);
        return {
            data: error.message,
            success: true,
