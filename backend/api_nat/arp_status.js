...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - ARP - New Running data status cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    console.log("NAT - ARP - Use date in NewStatusGetDailyStatusReport...", dateToday, dateTomorrow);
    try {
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                await sequelize.query(`
                    INSERT INTO [NAT_DX_TO_PICO].[dbo].[ARP_DAILY_STATUS_REPORT] ([operation_day],[is_operation_day],[process],[line_name],[machine_name],[status_name],[daily_duration_s],[daily_count],[shift1_duration_s],[shift1_count],[shift2_duration_s],[shift2_count],[shift3_duration_s],[shift3_count],[registered_at])
                    SELECT
                    ...
                    WHERE
                        NOT EXISTS ( 
                            SELECT 1
                            FROM [NAT_DX_TO_PICO].[dbo].[ARP_DAILY_STATUS_REPORT]
            ...
            console.log("NAT - ARP - Insert status new Done!");
            ...
            console.log("NAT - ARP - Can't new insert : Length = 0");
        }
    } catch (error) {
        console.log("NAT - ARP - new status insert error:", error);
        return {
            data: error.message,
            success: true,
