...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - GD2ND - Running data setting cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailySettingReport = async () => {
    try {
        let data = await dbNAT.query(`
            WITH [rn] AS (
                SELECT DISTINCT
                    ...
                    CONCAT('LINE ', CAST(LEFT(RIGHT(a.[mc_no],3),2) AS INT)) AS [line_no],
                    1 AS [mc_order],
                ...
                FROM [nat_mc_mcshop_2gd].[dbo].[DATA_PRODUCTION_2GD] a
                LEFT JOIN [nat_mc_mcshop_2gd].[dbo].[DATA_MASTER_2GD] b ON a.mc_no = b.mc_no
            )
            SELECT * FROM [rn]
            where rn = 1
            ORDER BY [mc_no]
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                const check = await sequelize.query(
                  `
                    SELECT [process]
                            ...
                            ,[registered_at]
                        FROM [NAT_DX_TO_PICO].[dbo].[GD2ND_SETTING]
                    WHERE machine_name = ?
                  `,
                  {
                    replacements: [mc_no],
                ...
                if (check.length === 0) {
                  ...
                  await sequelize.query(
                    `
                    INSERT INTO [NAT_DX_TO_PICO].[dbo].[GD2ND_SETTING] ([process], [line_name], [machine_name], [machine_order], [shift1_start_time], [count_factor], [target_cycle_time_ms], [registered_at])
                    VALUES (?, ?, ?, ?, ?, ?, ?, GETDATE())
                    `,
                    {
                  ...
                  if (!isSame) {
                    console.log("NAT - GD2ND - !isSame", !isSame);
                    
                    // ไม่เหมือนกัน → del แล้ว Insert ใหม่
                    await sequelize.query(
                      `
                      DELETE FROM [NAT_DX_TO_PICO].[dbo].[GD2ND_SETTING] WHERE machine_name = ?;
                      `,
                      {
                        replacements: [mc_no],
                      }
        ...
        console.log("NAT - GD2ND - status insert error:" , error);
        return {
            data: error.message,
            success: true,
