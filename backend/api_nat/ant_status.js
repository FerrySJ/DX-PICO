...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NAT - ANT - New Running data status cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    console.log("NAT - ANT - Use date in NewStatusGetDailyStatusReport...", dateToday, dateTomorrow);
    try {
            ...
            WITH [base_alarm_r] AS (
                SELECT
                    ...
                    CASE
                        ...
                        ELSE 'before'
                    END AS [alarm_type]
                FROM [nat_mc_assy_ant_new].[dbo].[DATA_ALARMLIS_ANT]
                WHERE [occurred] BETWEEN @start_date_p1 AND @end_date_p1 AND ([alarm] LIKE '%REAR%' OR [alarm] LIKE 'GSSM%' OR [alarm] LIKE 'PLC%' OR [alarm] LIKE 'PLEASE%')
            ),
            [base_alarm_f] AS (
                SELECT
                    [mc_no],
                    ...
                    END AS [alarm_type]
                FROM [nat_mc_assy_ant_new].[dbo].[DATA_ALARMLIS_ANT]
                WHERE [occurred] BETWEEN @start_date_p1 AND @end_date_p1 AND ([alarm] LIKE '%FRONT%' OR [alarm] LIKE 'GSSM%' OR [alarm] LIKE 'PLC%' OR [alarm] LIKE 'PLEASE%')
            ),
			[merge] AS (
              SELECT
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                await sequelize.query(`
                    INSERT INTO [NAT_DX_TO_PICO].[dbo].[ANT_DAILY_STATUS_REPORT] ([operation_day],[is_operation_day],[process],[line_name],[machine_name],[status_name],[daily_duration_s],[daily_count],[shift1_duration_s],[shift1_count],[shift2_duration_s],[shift2_count],[shift3_duration_s],[shift3_count],[registered_at])
                    SELECT
                    ...
                    WHERE
                        NOT EXISTS ( 
                            SELECT 1
                            FROM [NAT_DX_TO_PICO].[dbo].[ANT_DAILY_STATUS_REPORT]
            ...
            console.log("NAT - ANT - Insert status new Done!");
            return {
                data: data[0],
                success: true,
            ...
            console.log("NAT - ANT - Can't new insert : Length = 0");
        }
    } catch (error) {
        console.log("NAT - ANT - new status insert error:", error);
        return {
            data: error.message,
            success: true,
