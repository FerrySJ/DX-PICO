...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - MBR - Running data setting cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailySettingReport = async () => {
    try {
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                if (check.length === 0) {
                  ...
                  if (!isSame) {
                    console.log("NHT - MBR - !isSame", !isSame);
        ...
        console.log("NHT - MBR - status insert error:" , error);
        return {
            data: error.message,
            success: true,
