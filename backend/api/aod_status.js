...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - AOD - New Running data status cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
    ...
    try {
        let data = await sequelize.query(`
            ...
            WITH [base_alarm] AS (
                SELECT
                    ...
                    CASE
                        ...
                        ELSE 'before'
                    END AS [alarm_type]
                FROM [data_machine_aod].[dbo].[DATA_ALARMLIS_AOD]
            ...
            [base_monitor_iot] AS (
                ...
                FROM [data_machine_aod].[dbo].[MONITOR_IOT]
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                await sequelize.query(`
                    INSERT INTO [NHT_DX_TO_PICO].[dbo].[AOD_DAILY_STATUS_REPORT] ([operation_day],[is_operation_day],[process],[line_name],[machine_name],[status_name],[daily_duration_s],[daily_count],[shift1_duration_s],[shift1_count],[shift2_duration_s],[shift2_count],[shift3_duration_s],[shift3_count],[registered_at])
                    SELECT
                    ...
                    WHERE
                        NOT EXISTS ( 
                            SELECT 1
                            FROM [NHT_DX_TO_PICO].[dbo].[AOD_DAILY_STATUS_REPORT]
            ...
            console.log("NHT - AOD - Insert status new Done!");
            return {
                data: data[0],
                success: true,
            ...
            console.log("NHT - AOD - Can't new insert : Length = 0");
        }
    } catch (error) {
        console.log("NHT - AOD - new status insert error:", error);
        return {
            data: error.message,
            success: true,
