...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - MBR - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NHT - MBR - prod...", dateToday, dateTomorrow);

    try {
        let data = await sequelize.query(`
            DECLARE @Columns NVARCHAR(MAX);
            DECLARE @Database NVARCHAR(MAX);
            DECLARE @DatabaseLine NVARCHAR(MAX);
            DECLARE @SQL NVARCHAR(MAX);

            SET @Columns = '[daily_tt]';
            SET @Database = '[data_machine_assy1].[dbo].[DATA_PRODUCTION_ASSY]';
            SET @DatabaseLine = '[data_machine_assy1].[dbo].[master_mc_run_parts]';
            ...
            SET @SQL = '
                ...
                SELECT
                    ...
                    MAX(CONCAT(''LINE '', line_no)) AS [line_name],
                ...
                LEFT JOIN ' + @DatabaseLine + ' m ON s.[mc_no] = m.[mc_no]
                GROUP BY
                    [operation_day],
                    s.[process],
                    s.[mc_no]
        ...
        console.log("NHT - MBR - prod insert error:", error);
        return {
            data: error.message,
            success: true,
