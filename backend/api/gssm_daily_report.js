...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - GSSM - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NHT - GSSM - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await sequelize.query(`
            ...
            SET @Columns = '[total]';
            SET @Database = '[data_machine_gssm].[dbo].[DATA_PRODUCTION_GSSM]';
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NHT_DX_TO_PICO].[dbo].[GSSM_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NHT_DX_TO_PICO].[dbo].[GSSM_DAILY_REPORT]
                        WHERE
        ...
        console.log("NHT - GSSM - prod insert error:", error);
        return {
            data: error.message,
            success: true,
