...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - FIM - Running data setting cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
    ...
    try {
        let data = await sequelize.query(`
            WITH [rn] AS (
                ...
                FROM [data_machine_fim].[dbo].[DATA_PRODUCTION_FIM] a
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                if (check.length === 0) {
                  ...
                  if (!isSame) {
                    console.log("NHT - FIM - !isSame", !isSame);
        ...
        console.log("NHT - FIM - status insert error:" , error);
        return {
            data: error.message,
            success: true,
