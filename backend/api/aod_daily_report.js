...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - AOD - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    try {
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NHT_DX_TO_PICO].[dbo].[AOD_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NHT_DX_TO_PICO].[dbo].[AOD_DAILY_REPORT]
                        WHERE
        ...
        console.log("NHT - AOD - prod insert error:", error);
        return {
            data: error.message,
            success: true,
