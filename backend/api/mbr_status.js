...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("MBR - New Running data status cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    console.log("MBR - Use date in NewStatusGetDailyStatusReport...", dateToday, dateTomorrow);
    try {
            ...
            WHERE
                NOT EXISTS (
                    SELECT
                        1
                    FROM
            [NHT_DX_TO_PICO].[dbo].[MBR_DAILY_STATUS_REPORT]
                    WHERE
...
`
            ...
            console.log("MBR - Insert new Done!");
            ...
            console.log("MBR - Can't new insert : Length = 0");

        }

    } catch (error) {
        console.log("MBR - new status insert error:", error);
        return {
            data: error.message,
            success: true,
