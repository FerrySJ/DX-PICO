...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - AN - Running data reprod cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const getDailyReport = async (dateQuery) => {
    ...
    console.log("NHT - AN - prod...", dateToday, dateTomorrow);
    ...
    try {
        let data = await sequelize.query(`
            ...
            SET @Columns = '[tt]';
            SET @Database = '[data_machine_an2].[dbo].[DATA_PRODUCTION_AN]';
        ...
        if (data[0].length > 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                await sequelize.query(`
                    INSERT INTO  [NHT_DX_TO_PICO].[dbo].[AN_DAILY_REPORT] (
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1
                        FROM  [NHT_DX_TO_PICO].[dbo].[AN_DAILY_REPORT]
                        WHERE
        ...
        console.log("NHT - AN - prod insert error:", error);
        return {
            data: error.message,
            success: true,
