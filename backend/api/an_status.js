...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    try {
        let data = await sequelize.query(
            ...
            -- เปลี่ยนวันที่ด้วย
            DECLARE @end_date DATETIME = CASE WHEN @TargetEndDate > GETDATE () THEN
                GETDATE ()
            ELSE
                @TargetEndDate
            END;
            ...
            [status_alarm] ORDER BY[occurred]) AS[occurred_next],
                LEAD([alarm_type]) OVER (PARTITION BY[mc_no],
            [status_alarm] ORDER BY[occurred]) AS[next_type]
            FROM
            [base_alarm]
            ),
            [paired_alarms] AS (
            ...
            [broker_f],
                    ...
                    LAG(
                        ...
                        ELSE
                            0
                        END) OVER (PARTITION BY[mc_no] ORDER BY[registered]) AS[prev_is_zero],
                LEAD(
                    CASE WHEN[broker_f] = 0 THEN
                        1
                    ELSE
                        0
                    END) OVER (PARTITION BY[mc_no] ORDER BY[registered]) AS[next_is_zero],
                LEAD([registered]) OVER (PARTITION BY[mc_no] ORDER BY[registered]) AS[next_registered]
            FROM
            [base_monitor_iot]
            ),
            [flagged] AS (
            ...
            [grpz] AS (
                SELECT
                    *,
                    SUM(
                        CASE WHEN[start_flag] = 1 THEN
                            1
                        ELSE
                            0
                        END) OVER (PARTITION BY[mc_no] ORDER BY[registered] ROWS UNBOUNDED PRECEDING) AS[grp]
            FROM
            [flagged]
                WHERE
            [is_zero] = 1
            ...
            [mc_no],
                    ...
                    MAX(
                        CASE WHEN[end_flag] = 1 THEN
                            ISNULL ([next_registered],[registered])
                        END) AS[occurred_end]
                FROM
            [grpz]
                GROUP BY
            ...
            [date] AS[operation_day],
                    ...
                    CAST(
                    RIGHT ([mc_no], 2) AS INT) AS line_name,
