...
cron.schedule('1 7 * * *', async () => {
    ...
    console.log("NHT - AN - New Running data status cron job for date:", dateToday, hours, moment().tz('Asia/Bangkok').format("YYYY-MM-DD HH:mm:ss"));
}, {
...
const NewStatusGetDailyStatusReport = async (dateQuery) => {
    ...
    console.log("NHT - AN - Use date in NewStatusGetDailyStatusReport...", dateToday, dateTomorrow);
    try {
        let data = await sequelize.query(`
            ...
            WITH [base_alarm] AS (
                SELECT
                    ...
                    CASE
                        ...
                        ELSE 'before'
                    END AS [alarm_type]
                FROM [data_machine_an2].[dbo].[DATA_ALARMLIS_AN]
            ...
            [base_monitor_iot] AS (
                ...
                FROM [data_machine_an2].[dbo].[MONITOR_IOT]
        ...
        if (data[0].length > 0) {
            ...
            for (let index = 0; index < result.length; index++) {
                await sequelize.query(`
                    ...
                    WHERE
                        NOT EXISTS ( 
                            SELECT 1
                            FROM [NHT_DX_TO_PICO].[dbo].[AN_DAILY_STATUS_REPORT]
            ...
            console.log("NHT - AN - Insert status new Done!");
            return {
                data: data[0],
                success: true,
            ...
            console.log("NHT - AN - Can't new insert : Length = 0");
        }
    } catch (error) {
        console.log("NHT - AN - new status insert error:", error);
        return {
            data: error.message,
            success: true,
